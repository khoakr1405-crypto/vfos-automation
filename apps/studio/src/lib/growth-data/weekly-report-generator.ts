...
import { loadAffiliateCtaPlans, loadChannels, loadPublishedPosts } from './load';
import { resolveInsideRepo } from './paths';
import { readApiRuntimeStore, readRuntimeStore } from './runtime-store';
...
export function getIsoWeekRange(weekId: string): { start: Date; end: Date } {
...
`;
  ...
  if (!dryRun) {
    ...
    if (!jsonAbsPath || !mdAbsPath) {
      throw new Error('Lỗi định vị thư mục lưu trữ báo cáo.');
    }
    ...
    try {
      ...
      jsonFilePath = jsonRelPath;
      markdownFilePath = mdRelPath;
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : String(err);
      throw new Error(`Không thể ghi file báo cáo ra đĩa: ${msg}`);
    }
  }
  ...
  return {
    ...
    actionPlanCount: report.actionPlan.length,
    report,
