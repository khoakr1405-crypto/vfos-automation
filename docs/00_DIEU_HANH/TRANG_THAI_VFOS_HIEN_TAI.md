...
> **Cập nhật lần cuối**: 2026-07-11 (**Trend Scout R2 UI (Phần 50)**: commit `e0788dc` CHƯA PUSH — 7 file: `scout.ts` server lib pure-read + spawn detached, GET `/api/studio/entertainment/scout` pool+filter ?niche, POST `/api/studio/entertainment/scout/run` quét detached, `ScoutPanel` thu gọn trong Action 1 "Tải link" /lanes/content, dropdown ngách = bộ lọc shortlist, guard chung Douyin profile, poll 5s, promote qua intake 409 sẵn có; fix intake 502 surface message thật — root cause lỗi Operator = CAPTCHA transient DOUYIN_SETUP_REQUIRED, KHÔNG phải thiếu channelId; verify sống: 409 message chuẩn + promote thành công job thật ent_fishing_20260711_154233 INTAKE_DONE 491s. R1 CLI `pnpm ent:scout` commit `3384f40` CHƯA PUSH — 8 file: config/scout fishing+cooking, scout-core thuần + test 25/25, douyin-search capture, scout-jobs-index, 30-scout.ts, alias; smoke live PASS không CAPTCHA, snapshot thật scout_cooking_20260710_185800 3 ứng viên. Cả 2 commit CHƯA PUSH. **End-to-end montage smoke anchors (Phần 49)**: round **CÓ render, no publish** — chạy `ENT_MONTAGE_ENGINE=anchors pnpm tsx scripts/ent-vlog/10-montage-v2.ts --id ent_scratch_anchors` trên job scratch (copy từ `ent_fishing_20260625_222949`) → output `montage_v2_short.mp4` **47,145,226 bytes · 92.00s**; log dòng 1 `[10] Anchors (catch_moments): 199.5, 318.5, 353.5, 437.5` (KHÔNG có STORY engine, KHÔNG fallback hardcode); đã **xóa story_arc.json** khỏi scratch mà vẫn render PASS → anchors KHÔNG phụ thuộc story; job thật KHÔNG bị đụng (mtime/size không đổi); **git status clean** (scratch+log gitignored). **Kết luận: anchors engine SỐNG end-to-end** (data layer Phần 48 + render Phần 49) — không cần fix. KHÔNG sửa code/config/channel/UI toggle, KHÔNG publish, KHÔNG commit code; proof scratch+log đang GIỮ, cleanup để round riêng. Trước đó **Smoke-test anchors data layer (Phần 48)**: commit `90f8933` ĐÃ PUSH — test mới `tests/entertainment-anchors-engine.test.ts` chứng minh anchors data layer (đường non-story) còn **SỐNG**: `deriveAnchors` (5 catch_moments thật `ent_fishing_20260625_222949` → đúng `[199.5,318.5,353.5,437.5]`; cluster+pick-best; top-6/sort; empty→`[]`) + `readAnchorPlan` (anchors.json→catch_moments→FALLBACK đúng thứ tự; KHÔNG rơi hardcode khi có artifact thật); anchors-engine **11/11** PASS · story-engine regression **16/16** PASS · Biome PASS; fixture hermetic `data/temp` (gitignored) tự dọn; KHÔNG sửa engine/config/job/channel/pipeline, KHÔNG render/publish; end-to-end montage smoke `ENT_MONTAGE_ENGINE=anchors` vẫn là round riêng **CÓ render, CHƯA chạy**. Trước đó **Channel Overview storyEngine chip (Phần 47)**: commit `39590be` ĐÃ PUSH — ChannelCard có chip **read-only** `Engine: Story/Anchors`, storyEngine surface qua EntChannelUi/listChannelsForUi/EntChannelLite/ChannelCard; verify UI thật card Vlog Câu cá = `Engine: Story`; typecheck/Biome/test **16/16** PASS; read-only, KHÔNG config/createJob/toggle/mutation/anchors/render/publish. Story-engine 4 tầng (`bd30edb`·`136b209`·`53aace2`·`39590be`). Trước đó **Per-channel storyEngine safe-mode (Phần 46)**: commit `53aace2` ĐÃ PUSH — channel config có `storyEngine` (ch_fishing=`story`), `EntChannel` += field, `createJob` copy engine từ channel qua `jobStoryEngineFor`; guard 2 lớp (coerceChannel + jobStoryEngineFor) thiếu/sai/null → `story`; config toàn `story` chưa bật anchors; typecheck/Biome/test **15/15** PASS; KHÔNG đụng pipeline/anchors/UI toggle/publish guard, KHÔNG render/publish; UI toggle mutation + Channel Overview read-only + anchors smoke-test vẫn defer. Trước đó **UI indicator story badge (Phần 45)**: commit `136b209` ĐÃ PUSH — Production Panel có badge **READ-ONLY** Engine·Phân loại·Độ tin (chỉ `production-panel.tsx`); verify UI thật 2 job (vừa+cao) + default "Chưa phân loại"; KHÔNG toggle/mutation/gate/render/publish; per-channel + UI toggle mutation + anchors smoke-test vẫn defer. Trước đó **Story-flag wiring per-job (Phần 44)**: commit `bd30edb` ĐÃ PUSH — per-job `storyEngine` field trong manifest (default `story`) + `resolveMontageEngine` defensive (chỉ `anchors` tường minh mới ra anchors) + surface `story_confidence/source_type` vào manifest cho UI; test mới `entertainment-story-engine` **9/9** + typecheck/Biome PASS; KHÔNG render/publish; chưa mở anchors/per-channel/UI toggle. **ĐÍNH CHÍNH**: story-flag đã commit `8e20424` (KHÔNG phải chưa commit), multichannel R1+R2 đã code/push `43e65d6`+`cf3cb83` (KHÔNG phải chưa code). Trước đó **Round hygiene (Phần 43)**: dọn 3 file dirty out-of-scope trên `feat/ent-multichannel` — `bgm_library.json` REVERT về seed (runtime drift), `_hookframes/` DELETE + gitignore (commit `d92db11`), `implementation_plan.md` DELETE → **working tree CLEAN**, branch **local == origin tại `d92db11`**; publish vẫn CHƯA chạy. Trước đó **Khóa sổ round lane câu cá (Phần 42)**: 3 commit `413e0ac` (scrub recall tune) + `19ed0ba` (caption lower-third) + `0f2924a` (self-repair loop step 13, giữ nguyên 15 cổng QA, 5/5 PASS) đều **ĐÃ PUSH `origin/feat/ent-multichannel` — local == origin** (verify sync); deliverable `montage_v2_short_ambient.mp4` Operator duyệt **ĐẠT**; **publish CHƯA chạy — vẫn là cổng duyệt riêng**. Trước đó **Caption lower-third + verify deliverable end-to-end (Phần 41)**: fix `centerPresetOnBand` neo caption vào dải dưới (kẹp 0.74–0.86) thay vì median mọi dải → commit `19ed0ba` pushed; render thật clip 222949 (KHÔNG publish) → caption lower-third + 0 CJK + hook-verify PASS, Operator duyệt ĐẠT; 2 commit `413e0ac`+`19ed0ba` đã chạy pipeline đầy đủ. Phát hiện flaky script-QA step 13 (gpt-5.5 trượt 1 cổng/lần) → mở round repair-loop. Trước đó 2026-06-29: **POC scrub chữ Trung (Evidence Gate, 6 clip thật) → giữ `delogo`; recall tune `--fps 3 --min-score 0.5` ĐÃ ÁP vào `10-montage-v2.ts` lane câu cá (222949 sót 12.5%→5.4%, không FP), CHƯA commit; inpaint Option D defer — xem Phần 40**. Trước đó 2026-06-24: **Phase 3 lane Giải trí — NỐI NÚT "ĐĂNG TIKTOK" TRÊN UI STUDIO THẬT (Phần 39)**: đưa 11 file Phase 3 vào `feat/entertainment-lane` (nhánh chạy Studio có .env+data) → UI→route→`publishToTikTok`→client thật; commit `9322f9f` (+1714/−92, KHÔNG đụng 5 dirty WIP). FIX bug `node:fs` edge: bỏ `instrumentation.ts`, nạp `.env` gốc qua `next.config.ts` (Node, không webpack) — **phải restart Studio 3002**. Xác minh Next dev: readiness 200 `tiktokApiReady:true`, publish guard 409 ALREADY_POSTED (không đăng trùng), 404 control; biome/tsc/test sạch. Trước đó: **ROUND 2 (ĐĂNG THẬT) HOÀN TẤT, DoD ĐẠT**: job `ent_squid_001` đăng thật 1 video qua TikTok Content Posting API → state `TIKTOK_POSTED`, `publishId=v_pub_file~v2-1.7654841036543494165`, đăng **SELF_ONLY** vì app chưa audit. Giải blocker `unaudited_client_can_only_post_to_private_accounts` = bật **Private account** (không phải lỗi code). Thêm `creator_info/query` + **multi-chunk upload** (70MB→6 chunk) + CLI runner `tiktok-publish-run.ts`; commit `3eb6788` worktree (CHƯA push, no merge/PR). Validation: biome 0 · node:test 30/30. access_token 24h, refresh_token 365 ngày (`pnpm tiktok:oauth refresh`). Xem **Phần 38**. Round 1 (nền+mock) commit `f9a2fa6`. Trước đó: lane Giải trí MERGED master `e2d0a55` — Phần 37.)
> **Branch**: master `e2d0a55` (lane Giải trí end-to-end). **Phase 3 round 1**: `feat/entertainment-tiktok-publish` (`f9a2fa6`, base master, **đã push origin, CHƯA merge/PR** — DoD cần đăng thật Round 2). Nhánh dev `feat/entertainment-lane` (`5fde46e`, đa-lane archive). | **🆕 Lane câu cá (round hiện tại)**: `feat/ent-multichannel` HEAD **`e0788dc` — local ahead 2, CHƯA PUSH** (working tree CLEAN sau 2 commit Trend Scout R1+R2); chứa `413e0ac`+`19ed0ba`+`0f2924a`+`bd30edb`+`136b209`+`53aace2`+`39590be`+`90f8933` (test anchors smoke) + state-docs `d1239f8`/`d92db11`/`257ffb5`/`d1a9a8e`/`7705bb1`/`ba19465`/`1049dac` (Phần 42–50). **Phần 49** = smoke render anchors trên scratch (CÓ render, no publish, no commit code) — proof `data/temp/ent/ent_scratch_anchors/` + `_anchors_smoke.log` đang giữ (gitignored). | **Commit mốc**: `f9a2fa6` (`feat(ent-lane): add guarded TikTok publish flow (Phase 3 round 1)`). ⚠️ 5 file dirty NGOÀI SCOPE vẫn treo trên dev branch (`source-intake/route.ts`, `source-url/`, `package.json`, `bgm_library.json`, `implementation_plan.md`) — CHƯA xử lý; 3 dirty out-of-scope trên `feat/ent-multichannel` (`bgm_library.json`, `_hookframes/`, `implementation_plan.md`) **ĐÃ XỬ LÝ xong** (round hygiene Phần 43, commit `d92db11`) → **working tree CLEAN**.
