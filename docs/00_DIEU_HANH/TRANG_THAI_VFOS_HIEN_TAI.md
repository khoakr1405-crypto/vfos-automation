...
### G1-A#2 `13e6758` — Vá charset sub_id + FACTS VERIFIED từ dashboard Shopee (Operator screenshot 23/07)
- **Báo cáo chuyển đổi** (affiliate.shopee.vn): CÓ cột **`Sub_id`**, mức ĐƠN HÀNG (1 đơn/dòng); các cột: Chi tiết đơn hàng · Thông tin cửa hàng/sản phẩm/ưu đãi · Giá trị đơn hàng · Hoa hồng sản phẩm/đơn hàng · Phí quản lý MCN · Hoa hồng ròng TTLK · Sub_id · Thông tin bổ sung. Nút "Xuất dữ liệu" KHÓA khi 0 dòng → CSV thật chỉ có sau đơn đầu tiên.
- **Custom Link**: 5 ô **Sub_id1→Sub_id5** + "Lấy link"; nhận re-convert cả short link sẵn có; ràng buộc CỨNG **"chỉ chữ và số (a-z,A-Z,0-9)"** → vá `deriveVideoSubId`: `vfos_<jobId>` → **`vfos`+jobId-bỏ-separator** (vd `vfosjob20260625002`); khớp report bằng derive-rồi-so (không parse ngược). 40/40 test.
