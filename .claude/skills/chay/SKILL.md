...
### SHOPEE PRODUCT CARD — 10 field BẮT BUỘC

Khi vào Shopee-First Lane, agent phải tạo **Shopee Product Card** với 10 field. Lưu tại `production/batch_001/<video_id>/shopee_product_card.json`.

| # | Field | Mô tả | Nếu không có data |
|---|---|---|---|
| 1 | `shopee_product_url` | URL Shopee VN của sản phẩm (eg `https://shopee.vn/...`) | bắt buộc — nếu không có thì không lập card |
| 2 | `product_name` | Tên sản phẩm chính thức (theo listing Shopee) | bắt buộc |
| 3 | `price_vnd` | Giá hiện tại (VNĐ) | ghi `"unknown"` — KHÔNG bịa |
| 4 | `commission_pct` | % hoa hồng Shopee Affiliate (lấy từ Shopee Affiliate dashboard) | ghi `"unknown"` — KHÔNG bịa |
| 5 | `sales_count` | Số lượng đã bán (eg `"5k+"`, `"12.3k"`) | ghi `"unknown"` — KHÔNG bịa |
| 6 | `rating` | Rating trung bình (eg `4.8`) | ghi `"unknown"` — KHÔNG bịa |
| 7 | `review_count` | Số lượng review | ghi `"unknown"` — KHÔNG bịa |
| 8 | `shop_name` | Tên shop bán hàng trên Shopee | ghi `"unknown"` — KHÔNG bịa |
| 9 | `why_worthwhile` | Lý do đáng làm — phải bao gồm 5 điểm: (a) giải quyết vấn đề gì, (b) ai có thể mua, (c) visual demo có dễ hiểu không, (d) có phù hợp content-led affiliate không, (e) có tiềm năng chuyển đổi Facebook Reels VN không | bắt buộc — operator/agent tự viết, ngắn gọn |
| 10 | `data_confidence` | Mức độ tin cậy data: `high` (lấy được full price/commission/sales/rating từ Shopee trực tiếp) / `medium` (lấy được ≥3 field thật, còn lại unknown) / `low` (≥3 field unknown, dữ liệu chủ yếu từ user dán/category knowledge) | bắt buộc — phản ánh trung thực mức độ verify |

**Quy tắc bịa**: nếu không lấy được data trực tiếp (giá, hoa hồng, sales, rating, review, shop), **luôn ghi `"unknown"`**. Nếu ≥3 field trong (price/commission/sales/rating/review/shop) đều `unknown` → `data_confidence = "low"` + báo user, hỏi có dán thêm dữ liệu hay tiếp tục với data thiếu.
...
[ ] (Shopee-First Lane only) Shopee Product Card có đủ 10 field? Field unknown ghi "unknown", không bịa giá/hoa hồng/sales/rating/review/shop?
